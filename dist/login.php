...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT id, password, password2, usertype, CONCAT(fname, ' ', LEFT(mname, 1), '. ', lname) AS fullname FROM users WHERE username = ?");
    $stmt->bind_param("s", $username);
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows === 1) {
        $stmt->bind_result($id, $hashed_password, $hashed_password2, $usertype, $fullname);
        $stmt->fetch();
        if (password_verify($password, $hashed_password) || password_verify($password, $hashed_password2)) {
            $_SESSION['user_id'] = $id;
    ...
    <div class="login-card shadow-lg">
        ...
        <form method="POST" autocomplete="off" novalidate>
            ...
            <div class="form-floating mb-4 position-relative">
                ...
                <button type="button" id="togglePassword" tabindex="-1" style="position:absolute;top:50%;right:1rem;transform:translateY(-50%);background:transparent;border:none;outline:none;">
                    <i id="eyeIcon" class="bi bi-eye-slash" style="font-size:1.2rem;color:#888;"></i>
                </button>
            </div>
